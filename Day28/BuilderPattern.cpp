#include <iostream>
#include <stdexcept>
#include <string>
#include <vector>
#include <map>

using namespace std;

class HttpRequest{
private:
    string url;
    string method;
    map<string, string> headers;
    map<string, string> queryParams;
    string body;
    int timeout;

    // Private constructor - can only be accessed by the Builder
    HttpRequest(){}
    // Hence anyclient which want HttpRequest class cannot create class using this HttpRequest class

public:
    friend class HttpRequestBuilder; // this class eligibilty to access the private member of HttpRequest class
    // giving access to private members

    // method to execute the HTTP Request
    void execute(){
        cout<<"Executing "<<method<<" request to "<<url<<endl;

        if(!queryParams.empty()){
            cout<<"Query Parameters "<<endl;
            for(const auto& param: queryParams){
                cout<<" "<<param.first<<"="<<param.second<<endl;
            }
        }

        cout<<"Headers: "<<endl;

        for(const auto& header: headers){
            cout<<" "<<header.first<<": "<<header.second <<endl;
        }

        if(!body.empty()){
            cout<<"Body: "<<body<<endl;
        }

        cout<<"Timeout: "<<timeout<<" seconds"<<endl;
        cout<<"Request executed successfully!"<<endl;
    }
};

// Builder class as a nested class
class HttpRequestBuilder{ //single responsibility of the builder is to create class of the target
private:
    HttpRequest  req;

public:
    // Method chaining 
    HttpRequestBuilder& withUrl(const string& u){
        req.url = u;
        return *this; //httpRequestBuilder  ka reference milega , req mai url set hojayega , but abhi hame object nhi milega HttpRequest ka we will get reference of HttpRequestBuilder
    }

    HttpRequestBuilder& withMethod(string method){
        req.method = method;
        return *this;
    }

    HttpRequestBuilder& withHeader(const string& key, const string& value){
        req.queryParams[key] = value;
        return *this;
    }

    HttpRequestBuilder& withQueryParams(const string& key, const string& value){
        req.queryParams[key] = value;
        return *this;
    }

    HttpRequestBuilder& withBody(const string& body){
        req.body = body;
        return *this;
    }

    HttpRequestBuilder&  withTimeout(int timeout){
        req.timeout = timeout;
        return* this;
    }

    // Build method to create immutable HttpRequest object
    HttpRequest build(){
        // validation logic can be added here
        if(req.url.empty()){
            throw runtime_error("URL cannot be empty");
        }
        return req;
    }
};

int main(){
    // Using Builder Pattern (nested class)
    HttpRequest request = HttpRequestBuilder()
        .withUrl("https://api.example.com")//give us HttpRequestBuilder ka object
        .withMethod("POST")
        .withHeader("Content-Type", "application/json")
        .withHeader("Accept", "application/json")
        .withQueryParams("key", "12345")
        .withBody("{\"name\": \"Aditya\"}")
        .withTimeout(60)
        .build();

    // watch video at the 50 min

    // second problem was mutability 
    // now since we dont have setter we cant change these values and to change we have to basically make the builder again 
    request.execute(); // Guaranteed to be in a consistent state
} 