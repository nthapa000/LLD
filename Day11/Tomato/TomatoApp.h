...
// SRP breaks
// principle of least knowledge breaks
...
public:
    ...
    void initializeRestaurants(){
        // just happy flow in reality we will have a CRUD operation
        ...
        // Taaki restaurant manager ki list mai ye restaurant add hojaye
    ...
    void addToCart(User* user, const string& itemCode){
        ...
        // Tomato app task is to delegate
        for(const auto& item: restaurant->getMenu()){
            // go through menu of restaurant
    ...
    // decide on run time that what type of order we want to place
    ...
    Order* checkout(User* user, const string& orderType, PaymentStrategy* paymentStrategy, OrderFactory* orderFactory){
        ...
        // order will be created and we passed all the parameter
