// Questions in which there are small small object prefer topdown approach
#include <iostream>
#include <string>
#include <vector>
#include <map>
...
public:
    ...
    // Calculate distance in kilometer between two locations using Haversine formula
...
class UserProfile{
    ...
    public:
        ...
        void removeInterest(const string& name){
            auto it = find_if(interests.begin(),interests.end(),[&name](const Interest* interest){
...
public:
    User(const string &userId){
        ...
        // made it tightly coupled since it wont change
    ...
    // current user nai like kiya hai ki nhi iss user ko
    ...
    // i dont want someone who have access to User should know about the profile class
        profile->display();
    }
};
...
// Forward declaration of Matcher interface
class Matcher{
public:
    virtual ~Matcher(){}
...
private:
    ...
    DatingApp(){
        // Default location-based matcher
...
public:
    ...
    std::vector<User*> findNearbyUsers(const std::string& userId, double maxDistance = 5.0){
        ...
        // Find users withing maxDistance km
        vector<User*> nearbyUsers = LocationService::getInstance()->findNearbyUsers(user->getProfile()->getLocation(),maxDistance,users);
