...
public:
    ...
    double getTotalOwed(){
        ...
        for(const auto& balance : balances){
            if(balance.second < 0){ // amount is negative
...
public:
    ...
    Expense(const string& desc, double amount, const string& paidBy, vector<Split>& splits, const string group=""){
...
public:
    ...
    string groupId;
    string name;
    vector<User*> members; // Observers list
    ...
    bool removeMember(const string& userId){
        ...
        // Remove this user from other member's balance maps
    ...
    // Update the Balance within group
    ...
    bool canUserLeaveGroup(const string& userId){
        if(!isMember(userId)){
            throw runtime_error("user is not part of this OG group");   
        }
    ...
    // get users balance within this group
    ...
    bool addExpense(string& description, double amount, string& paidByUserId, vector<string>& involvedUsers, SplitType splitType, const vector<double>& splitValues = {}){
        ...
        vector<Split> splits = SplitFactory::getSplitStrategy(splitType)->calculateSplit(amount, involvedUsers, splitValues);
        ...
        cout << "Expense added to " << name << ": " << description << " (Rs " << amount<< ") paid by " << paidByName <<" and involved people are : "<< endl;
