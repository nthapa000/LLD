...
// Base class for snake and ladder (both have start and end postiion)
class BoardEntity{
protected:
    int startPosition;
    int endPosition;
...
public:
    ...
    // we'll see it afterwards since we havent made the strategy class
...
public:
// since it is specific for this class
    enum Difficulty{
        EASY,
        MEDIUM,
        HARD // more snakes, less ladders
    };
        ...
        for(int i=0; i<totalEntities; i++){
            ...
            if(prob < snakeProbability){
                ...
                while(attempts < 50){
                    ...
                    // possibility is that we may get false that we cant insert entity here
                    if(board->canAddEntity(start)){
                        board->addBoardEntity(new Snake(start, end));
                        break;
                    }
