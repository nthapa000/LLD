...
public:
    ...
    // baseamount leta hai and usme final discounted amount return kr deta hai
...
public: 
    FlatDiscountStrategy(double amt){
        // assign the amount to its constructor
        amount = amt;
    }
...
public:
    // will get us the instance of it
    ...
    DiscountStrategy* getStrategy(StrategyType type, double param1, double param2 = 0.0) const { //param2 is optional
        // now when we require to change the StrategyType we will be breaking the open close principle
        ...
        if(type == StrategyType::PERCENT_WITH_CAP){ //we need perecentage and also uppervalue
...
public:
    ...
    void addProduct(Product* prod, int qty = 1){
        ...
        orginalTotal += item->itemTotal();
        currentTotal += item->itemTotal();
    }

    double getOrginalTotal(){
        return orginalTotal;
    }
...
class Coupon{
private:
// Coupon ki linked list bani hui hai
    Coupon* next;
...
public:
    ...
    void applyDiscount(Cart* cart){
        if(isApplicable(cart)){
            ...
            cout<<name()<< " applicable "<<discount<<endl;
            if(!isCombinable()){
                return;
            }
        }
...
public:
    ...
    // poori cart mai apply hota hai
    double getDiscount(Cart* cart) override {
        return strat->calculate(cart->getCurrentTotal());
    }

    // to tell konsa coupon lagaya hai
    string name() override {
...
public:
    ...
    bool isApplicable(Cart* cart) override {
        return cart->getOrginalTotal() >= threshold;
    }
...
public:
    ...
    bool isApplicable(Cart *cart) override{
        return cart->getPaymentBank() == bank && cart->getOrginalTotal() >= minSpend;
    }
...
public:
    ...
    void registerCoupon(Coupon* coupon) {
        ...
        if (!head) { // head null or not
            head = coupon;
        } else { 
            Coupon* cur = head;
            // gone till the tail of the LL 
            while (cur->getNext()) {
                cur = cur->getNext();
            }
            // assigned the coupon to tail of LL
            cur->setNext(coupon);
        }
    }
...
int main() {
    ...
    cout << "Original Cart Total: " << cart->getOrginalTotal() << " Rs" << endl;
