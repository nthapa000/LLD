...
private:
    ...
    int size;
    Symbol* emptyCell; //so that we dont have to handle null

public:
    Board(int s){
        size = s;
        emptyCell = new Symbol('_');
        // filled grid with emptyCell
...
public:
    ...
    // If all calls are filled and no winner
...
public:
    static TicTacToeGame* createGame(GameType gt, int boardSize){
        TicTacToeGame* game = new TicTacToeGame(boardSize);
        if(GameType::STANDARD == gt){
            game->setRule(new StandardTicTacToeRules());
            return game;
        }
        return nullptr;
    }
