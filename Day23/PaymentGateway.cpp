...
public:
    PaytmBankingSystem(){}
    // 20 percent chance for success and 80 percent change for failure
...
public:
    ...
    bool processPayment(double amount) override{
        ...
        // this was done so that we can see the retry mechanism working
        int r = rand() % 100;
        return r < 90;
    }
};
...
public:
    ...
    virtual bool processPayment(PaymentRequest* request){
        // paymentRequest mai process krta hai
        // it just said ki yeh yeh call hoga but didnt define them
        // child class function are called
...
public:
    PaytmGateway(){
        // bankingSystem ke andhar paytmBankingSystem daal diya
    ...
    bool confirmPayment(PaymentRequest* request) override{
        ...
        // Confirmation always suceeds in this simulation
        return true;
    }
};
...
public:
    ...
    PaymentGateway* getGateway(GatewayType type) {
        if (type == GatewayType::PAYTM) {
            PaymentGateway* paymentGateway = new PaytmGateway();//Paytm object
...
public:
    ...
    void setGateway(PaymentGateway* g) {
        if (gateway) delete gateway;//pehle sai set hai to delete kr deta tha
        gateway = g;
    }
...
// ----------------------------
class PaymentController { //all request go through it
private:
...
int main() {
    ...
    srand(static_cast<unsigned>(time(nullptr))); //random wali generates ho 
