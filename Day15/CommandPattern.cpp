...
public:
    ...
    void setCommand(int idx, Command* cmd){
        if(idx>=0 && idx < numButtons){
            // check whether ki uss button mai koi command already present hai ki nhi if already present then delete that first
    ...
    void pressButton(int idx){
        if(idx>=0 && idx<numButtons && buttons[idx]!= nullptr){
            ...
            cout<<"No command assigned to the button "<<idx<<endl;
        }
    }
